...
- **파일**: `circuit/src/services/projectConfig.ts` (450+ 줄)

**기능**:
- 프로젝트 메타데이터 저장
- AI 코딩 규칙 CRUD (생성/읽기/수정/삭제)
- 규칙 재정렬 (드래그 앤 드롭)
- Cursor `.cursorrules` 가져오기/내보내기
- 팀 설정 공유 준비
...
**파일**: Claude API 호출 부분 (conversationHandlers.ts 또는 유사 파일)

```typescript
// Claude API 호출 전
import { getAIRulesContext } from '@/services/projectConfig';

async function sendToClaude(workspacePath, userMessage) {
  // AI 규칙 가져오기
  const rulesContext = await getAIRulesContext(workspacePath);
...
### **우선순위 3: AI Context 통합**
- Claude API 호출 시 규칙 주입
- AI가 실제로 규칙을 따르는지 테스트
...
5. ⏳ **AI가 실제로 규칙을 따른다** (Claude API 통합 후)
...
**다음**: Electron 통합 → 빌드 → 테스트 → Phase 2 (검색, 테스팅, 패키지 관리)
