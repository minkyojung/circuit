...
import { Activity, CheckCircle2, XCircle, Clock, Filter, RefreshCw } from 'lucide-react';
...
export function MCPTimeline({
  ...
  const loadCalls = useCallback(async () => {
    try {
      ...
      const result = await ipcRenderer.invoke('circuit:history-get', query);
      ...
      console.error('[MCPTimeline] Failed to load calls:', err);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(loadCalls, refreshInterval);
    return () => clearInterval(interval);
  }, [loadCalls, refreshInterval]);
  ...
  const getStatusColor = (status: MCPCall['status']) => {
    switch (status) {
      case 'success':
  ...
  if (error) {
    return (
      <Stack space="4" className="p-4">
        ...
        <Button onClick={loadCalls} size="sm">
          Retry
        </Button>
      </Stack>
    );
  ...
  return (
    <Stack space="0" className="h-full overflow-hidden">
      ...
      <div className="flex items-center justify-between px-4 py-2 border-b border-sidebar-border bg-sidebar">
        ...
        <Button onClick={loadCalls} size="sm" variant="ghost">
