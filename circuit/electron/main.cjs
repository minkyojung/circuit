...
ipcMain.handle('circuit:watch-start', async (event, projectPath) => {
  try {
    ...
    const handleChange = (type, filePath) => {
      ...
      console.log('[Circuit] File change detected:', changeEvent);
    ...
    console.log('[Circuit] File watcher started');
    ...
    console.error('[Circuit] Watch start error:', error);
    return {
      success: false,
      error: error.message
    };
...
ipcMain.handle('circuit:watch-stop', async (event, projectPath) => {
  try {
    console.log('[Circuit] Stopping file watcher:', projectPath);

    if (activeWatchers.has(projectPath)) {
      const watcher = activeWatchers.get(projectPath);
      await watcher.close();
      activeWatchers.delete(projectPath);
      console.log('[Circuit] File watcher stopped');
    }
    ...
    console.error('[Circuit] Watch stop error:', error);
    return {
      success: false,
      error: error.message
    };
